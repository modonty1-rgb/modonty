...
export function computeArticleEntitySeo(article: ArticleSeoInput): EntitySeoScore {
  ...
  // rescue the number: averaging a zeroed meta with a perfect JSON-LD still read 50. The
  // block is checked BEFORE the weighting, not folded into it — a page Google will not show
  // has no performance to weigh, whatever its three dimensions score.
  const overall = isBlockedFromIndex(article.nextjsMetadata)
    ? 0
    : Math.round(meta.score * W_META + jsonLd.score * W_JSONLD + links.score * W_LINKS);
