...
- [x] **V6** — ✅ **تم الدفع والنشر والتحقّق (2026-06-05):** commit `ec7d021` → main (نسخة modonty 1.54.0) · باكب DB قبل الدفع (73 collections) · admin tsc صفر. **Vercel:** modonty-modonty **READY** · console **CANCELED** (ignoreCommand صح) · admin بنى (تكلفة بسيطة). **تحقّق حيّ على www.modonty.com (مقال كيما-زون حقيقي):** robots=index · canonical · الشريط+المعرض+اقرأ أيضاً · بلا شريط نهاية · JSON-LD (Article+Breadcrumb+@graph) · **صفر أخطاء console**. 🎉 المقال الجديد حيّ على الإنتاج وقابل للفهرسة.
  - **متبقّي (ritual، غير حرج):** سطر changelog للنسخة 1.54.0 (يُضاف من admin UI — كتابة DB، أجّلته تجنّباً لكتابة عمياء على prod). + مراقبة الميدان (Search Console + GA4) على مدى أيام.
