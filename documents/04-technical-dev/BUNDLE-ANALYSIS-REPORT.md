...
**Date**: 2026-02-07
**Tool**: `npx next experimental-analyze` (Next.js v16.1)
**Route**: `/` (Home page – Client bundle)
...
1. **App code (~447 KB)** – Largest share; review heavy imports, dynamic imports, tree-shaking
2. **Next.js (~263 KB)** – Framework baseline; expected
3. **react-dom (~61 KB)** – Core React runtime; expected
4. **lucide-react** – Present; `optimizePackageImports` configured
5. **chatbot/** – Visible; deferred loading should help
6. **tailwind-merge** – Small; minor impact

## Duplicated JavaScript (Chrome DevTools: 437 kB)

The analyzer does not show "duplicated" as separate metric. Likely causes:

- Same package in multiple chunks (shared runtime)
...
5. **Verify deferrals** – Confirm chatbot/GTM not loading before LCP
6. **Profile other routes** – e.g. `/articles/[slug]`, `/trending`
...
Opens at http://localhost:4000. Use route selector, Client/Server filter, and search to explore.
