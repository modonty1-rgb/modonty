...
export interface ModontyHeroImageUploadResult {
  heroImageUrl?: string;
  heroImageAlt?: string;
  heroImageCloudinaryPublicId?: string;
}

export async function uploadModontyHeroImage(
  imageData: ImageUploadData | null,
  slug: string
): Promise<{ success: boolean; result?: ModontyHeroImageUploadResult; error?: string }> {
  if (!imageData?.file) {
    return { success: true };
  }
  ...
  try {
    ...
    const publicId = `modonty/${slug}/hero/${seoFileName}`;
    ...
    formData.append("asset_folder", `modonty/${slug}/hero`);
    ...
    return {
      success: true,
      result: {
        heroImageUrl: optimizedUrl,
        heroImageAlt: altText.trim(),
        heroImageCloudinaryPublicId: cloudinaryPublicId,
      },
    };
  } catch (error) {
    console.error("Error in uploadModontyHeroImage:", error);
    return {
      success: false,
