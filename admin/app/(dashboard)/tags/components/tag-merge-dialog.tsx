...
export function TagMergeDialog({ source, candidates, open, onOpenChange, onMerged }: TagMergeDialogProps) {
  const { toast } = useToast();

  const [phase, setPhase] = useState<Phase>("form");
  const [search, setSearch] = useState("");
  const [pickerOpen, setPickerOpen] = useState(false);
  const [target, setTarget] = useState<TagLite | null>(null);
  const [impact, setImpact] = useState<TagMergeImpact | null>(null);
  const [confirmText, setConfirmText] = useState("");

  const [current, setCurrent] = useState(0);
  const [total, setTotal] = useState(0);
  const [items, setItems] = useState<ProgressItem[]>([]);
  const [recap, setRecap] = useState<{ moved: number; dedup: number; regenerated: number } | null>(null);
  ...
  useEffect(() => {
    ...
    let alive = true;
    getTagMergeImpact(source.id, target.id).then((res) => {
  ...
  const handleMerge = async () => {
    ...
    const prep = await prepareTagMerge({
      sourceId: source.id,
      targetId: target.id,
    ...
    await finalizeTagMerge({ sourceId: source.id, targetId: target.id });

    setRecap({ moved: prep.movedCount ?? 0, dedup: prep.dedupCount ?? 0, regenerated });
    setPhase("done");
    toast({
      title: "تمّ الدمج",
          ...
          <>
            <DialogHeader className="text-start">
              <DialogTitle className="flex items-center gap-2">
                ...
                دمج الوسم في وسم آخر
              </DialogTitle>
              <DialogDescription>تُنقل كل المقالات إلى الوجهة، ويصبح المصدر بصفر مقالات.</DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="mb-1.5 flex items-center gap-1 text-xs font-bold">
                    اختر وسم الوجهة <span className="text-red-500">*</span>
                  </label>
                  <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
                    ...
                    <PopoverContent align="start" className="w-[--radix-popover-trigger-width] p-0" dir="rtl">
                      ...
                      <div className="max-h-60 overflow-y-auto">
                        {filtered.length === 0 ? (
                          <div className="p-3 text-center text-xs text-muted-foreground">لا توجد وسوم مطابقة</div>
                        ) : (
                          filtered.map((c) => (
                            <button
                              key={c.id}
              ...
              <div className="space-y-3">
                {!target ? (
                  <div className="flex h-full min-h-40 flex-col items-center justify-center rounded-xl border border-dashed p-6 text-center">
                    ...
                    <p className="text-xs text-muted-foreground">اختر وسم الوجهة لعرض الأثر وتأكيد الدمج</p>
                  </div>
                ) : (
                  <>
                    {/* impact preview */}
                    <div className="rounded-xl border border-amber-500 bg-amber-500/10 p-3.5">
                      ...
                      {impact ? (
                        <ul className="space-y-1.5 text-[12.5px]">
                          ...
                          <li>
                            • <b className="tabular-nums">{impact.dedupCount}</b> مقال موسوم بالاثنين → يُدمج بلا تكرار.
                          </li>
                          <li>
                            • يُعاد توليد SEO + JSON-LD لـ<b className="tabular-nums">{impact.affectedCount}</b> مقال + صفحة الوجهة.
                          </li>
                          <li>
                            • يُسجَّل تحويل 308: <code dir="ltr" className="rounded bg-black/30 px-1 text-[11px] text-violet-500">/tags/{source.slug} → /tags/{target.slug}</code>
                          </li>
                          <li>• يصبح «{source.name}» بصفر مقال — تحذفه من الجدول عادي متى شئت.</li>
                        </ul>
                      ) : (
          ...
          <>
            ...
            <div className="space-y-2 text-[13px]">
              <div className="rounded-xl border bg-muted/40 px-3 py-2.5">
                <b className="tabular-nums">{recap.moved}</b> مقال نُقل · <b className="tabular-nums">{recap.dedup}</b> مكرّر دُمج بلا تكرار.
              </div>
              <div className="rounded-xl border bg-muted/40 px-3 py-2.5">
            ...
            <div className="rounded-xl border border-red-500/40 bg-red-500/10 px-3.5 py-3">
              <div className="text-sm font-bold">
                {source.name} <span className="ms-1 rounded-full border bg-background px-2 py-0.5 text-[11px] text-muted-foreground">٠ مقال</span>
              </div>
