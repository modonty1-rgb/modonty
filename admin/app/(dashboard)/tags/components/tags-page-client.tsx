...
import { AlertCircle, Hash, FileText, FolderOpen, AlertTriangle, type LucideIcon } from "lucide-react";
...
const KPIS: {
  ...
  { key: "all", label: "Total", icon: Hash, tone: "bg-muted text-foreground", ring: "ring-foreground/40", test: () => true },
  { key: "withArticles", label: "With articles", icon: FileText, tone: "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400", ring: "ring-emerald-500", test: (t) => t._count.articles > 0 },
  { key: "empty", label: "Empty", icon: FolderOpen, tone: "bg-slate-500/15 text-slate-600 dark:text-slate-400", ring: "ring-slate-500", test: (t) => t._count.articles === 0 },
  { key: "failing", label: "SEO < 100", icon: AlertTriangle, tone: "bg-red-500/15 text-red-600 dark:text-red-400", ring: "ring-red-500", test: (t) => t.seoScore < 100 },
];

export function TagsPageClient({ tags, missingSeoCount }: TagsPageClientProps) {
  ...
  const counts = useMemo(
    () => Object.fromEntries(KPIS.map((k) => [k.key, tags.filter(k.test).length])) as Record<KpiKey, number>,
    [tags],
  );
  const visible = useMemo(() => {
    const kpi = KPIS.find((k) => k.key === active) ?? KPIS[0];
    return tags.filter(kpi.test);
  }, [tags, active]);
  ...
  const handleBatchGenerate = async () => {
    ...
    try {
      ...
      const result = await batchGenerateTagSeo();
      toast({
        ...
        <div className="flex items-center justify-between rounded-lg border border-yellow-500/20 bg-yellow-500/[0.04] px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-yellow-500">
              {missingSeoCount} {missingSeoCount === 1 ? "tag" : "tags"} missing SEO cache
            </span>
          </div>
          <Button
            variant="ghost"
