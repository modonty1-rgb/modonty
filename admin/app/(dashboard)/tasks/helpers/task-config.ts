...
// The four columns, in board order. This array is the ONLY place the order lives:
// the board renders from it, the counters map over it, and the move menu offers
// every value except the card's own. Adding a column here adds it everywhere.
...
interface StatusMeta {
  label: string;
  /** URL segment for `/tasks/[status]`, and what the drop zone reports. */
  slug: string;
  /** Column header tint. Semantic per the admin entity standard (#3). */
  tone: string;
  dot: string;
}
...
export const TASK_PRIORITY_META: Record<TaskPriority, { label: string; tone: string }> = {
  LOW: { label: "هادية", tone: "bg-slate-500/15 text-slate-600 dark:text-slate-400" },
  NORMAL: { label: "عادية", tone: "bg-blue-500/15 text-blue-600 dark:text-blue-400" },
  HIGH: { label: "مهمة", tone: "bg-amber-500/15 text-amber-600 dark:text-amber-400" },
  URGENT: { label: "عاجلة", tone: "bg-red-500/15 text-red-600 dark:text-red-400" },
};
