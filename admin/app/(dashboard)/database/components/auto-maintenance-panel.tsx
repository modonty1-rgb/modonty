...
const STEPS: StepDef[] = [
  ...
  { key: "datePublished", label: "Published Articles Given a datePublished", description: "Published articles carrying no datePublished. Measured 28 Aug 2026 on production-synced data: 13 of 135 — live client articles for شركة جبر سيو and شركة جبر الجنوبية, all created 2026-04-09, before the publish mutation began stamping the field. datePublished is what Google reads inside Article: it is how a result gets a date beside it and one of the conditions an article result is judged on. Fills it from createdAt — the only date these rows carry (scheduledAt and ogArticlePublishedTime are empty, and dateModified is months later, so using it would claim the article was published after it was modified) — then regenerates the stored JSON-LD, because the card carries its own copy and fixing the column alone leaves Google reading the old one. Idempotent: a row that already has a date is never touched.", runner: runStepDatePublished },
