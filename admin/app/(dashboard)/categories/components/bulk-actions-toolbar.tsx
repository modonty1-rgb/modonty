...
import { bulkDeleteCategories } from "../actions/categories-actions";
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "Success",
          description: `Deleted ${selectedIds.length} categor${selectedIds.length === 1 ? "y" : "ies"}`,
        });
        onClearSelection();
        router.refresh();
      } else {
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete {selectedIds.length} categor{selectedIds.length === 1 ? "y" : "ies"} and all associated data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
