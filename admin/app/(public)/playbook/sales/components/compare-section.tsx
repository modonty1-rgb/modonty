...
export async function CompareSection() {
  ...
  return (
    <section id="compare" className="scroll-mt-6">
      <h3 className="text-[17px] font-bold leading-6">فيمَ نختلف عمّن حولنا</h3>
      <p className="mt-1.5 text-[13px] leading-6 text-muted-foreground">ست مقارنات يسمعها المندوب كل أسبوع، وجدول الفرق، وثلاث حالات لا نناسبها.</p>
      <div className="mt-2.5 space-y-2.5">
    <section className="mt-4 scroll-mt-6" id="comparison">
      <div className="mt-2.5 grid gap-2 xl:grid-cols-2">
        {battles.map((battle) => (
          <article key={battle.num} className="rounded-lg border bg-card p-2.5">
            ...
            <p className="mt-2 rounded-md bg-primary/[0.06] px-3 py-2 text-[13.5px] font-semibold leading-6 text-primary">{battle.keyLine}</p>
          </article>
        ))}
      </div>

      <div className="mt-2 overflow-hidden rounded-lg border">
        <div className="border-b bg-card px-2.5 py-3">
          ...
          <table className="w-full min-w-[860px] text-right text-[13px]">
            <thead>
              <tr className="border-b bg-muted/40">
                ...
                <th className="px-3 py-2.5 font-semibold text-muted-foreground">اشتراك برمجي عالمي</th>
