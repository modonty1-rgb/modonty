import { chromium } from '@playwright/test';
const base = 'http://localhost:3001';
const tag = process.argv[2] ?? '1';
const out = `C:/Users/w2nad/AppData/Local/Temp/claude/c--Users-w2nad-Desktop-dreamToApp-MODONTY/5c1fdcfb-a3ee-4967-aeb5-f15dc163736b/scratchpad/surfaces-${tag}.png`;
const browser = await chromium.launch();
const page = await (await browser.newContext({ viewport: { width: 1440, height: 1000 }, colorScheme: 'dark' })).newPage();
const errs = [];
page.on('pageerror', (e) => errs.push(String(e).slice(0, 120)));
await page.goto(`${base}/login`, { waitUntil: 'networkidle' });
await page.fill('input[type="email"]', 'claude-check@modonty.local');
await page.fill('input[type="password"]', 'Mdnty-Local-Check-2026!');
await page.click('button[type="submit"]');
await page.waitForTimeout(5000);
const r = await page.goto(base + '/playbook', { waitUntil: 'domcontentloaded', timeout: 180000 });
await page.waitForSelector('#surfaces', { timeout: 60000 });
await page.waitForTimeout(3000);
await page.locator('#surfaces').screenshot({ path: out });
const txt = await page.locator('#surfaces').innerText();
console.log('status', r?.status(), 'errs', errs.length);
console.log('hasOldLine', txt.includes('بلا طرف خارجي'));
await browser.close();
