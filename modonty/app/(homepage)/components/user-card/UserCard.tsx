...
export function UserCard() {
  ...
  if (!user) {
    return (
      <section className="relative overflow-hidden rounded-[1.35rem] border border-primary/30 bg-gradient-to-bl from-primary/25 via-card to-card p-3 shadow-[0_18px_34px_-28px_rgba(48,48,255,0.9)] lg:p-4">
        ...
        <Link href="/users/register" className="relative mt-3 block min-h-11 w-full rounded-xl bg-primary px-3 py-2.5 text-center text-sm font-bold text-primary-foreground shadow-[0_12px_24px_-14px_rgba(48,48,255,1)] transition-colors sm:hover:bg-primary/90 lg:mt-4">أنشئ حسابًا</Link>
      </section>
    );
  }
    ...
    <section aria-label="ملفك الشخصي" className="overflow-hidden rounded-2xl border border-primary/10 bg-card shadow-[0_10px_30px_-22px_rgba(14,6,90,0.45)]">
      ...
      <div className="px-3 pb-3 text-center sm:px-5 sm:pb-4">
        <div className="-mt-7 flex flex-col items-center gap-1 lg:-mt-8">
          <Avatar className="size-14 border-4 border-card shadow-sm lg:size-16">
        ...
        <h2 className="mt-1.5 truncate text-base font-bold text-foreground lg:mt-2">{user.name || "مستخدم مدونتي"}</h2>
        ...
        <div className="mt-3 grid grid-cols-3 divide-x divide-x-reverse divide-border border-t pt-2.5 lg:mt-4 lg:pt-3">
          {quickLinks.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} className="flex min-h-11 flex-col items-center justify-center gap-1 text-xs font-medium text-muted-foreground transition-colors sm:hover:text-primary">
