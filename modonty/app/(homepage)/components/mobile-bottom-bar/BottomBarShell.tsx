...
import { AskModoButton } from "@/app/(homepage)/components/mobile-bottom-bar/AskModoButton";
...
const DiscoverSheet = dynamic(
  () => import("./DiscoverSheet").then((m) => ({ default: m.DiscoverSheet })),
  { ssr: false }
);
const PartnersSheet = dynamic(
  () => import("./PartnersSheet").then((m) => ({ default: m.PartnersSheet })),
  { ssr: false }
);
interface BottomBarShellProps {
...
export function BottomBarShell({ categories, industries, tags, partners, services }: BottomBarShellProps) {
      ...
      >
        <div className="pointer-events-auto mx-auto flex h-14 w-fit items-center justify-center gap-0.5">
          ...
          <Sheet open={discoverOpen} onOpenChange={(open) => { if (open) setDiscoverMounted(true); setDiscoverOpen(open); }}>
            ...
            {discoverMounted && <DiscoverSheet categories={categories} industries={industries} tags={tags} activeCategory={activeCategory} onClose={() => setDiscoverOpen(false)} />}
          </Sheet>
          ...
          <Sheet open={partnersOpen} onOpenChange={(open) => { if (open) setPartnersMounted(true); setPartnersOpen(open); }}>
            ...
            {partnersMounted && <PartnersSheet partners={partners} activeClient={activeClient} onClose={() => setPartnersOpen(false)} />}
          </Sheet>
        </div>
      </nav>
