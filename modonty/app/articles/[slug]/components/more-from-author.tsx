...
export function MoreFromAuthor({ authorId, articleId, authorName }: MoreFromAuthorProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetchMoreFromAuthor(authorId, articleId)
      .then((data) => {
        setArticles(data ?? []);
        setFetched(true);
      })
  ...
  }, [open, fetched, authorId, articleId]);
  ...
  return (
    ...
    >
      {loading && <MoreFromAuthorSkeleton />}
      ...
      {!loading && fetched && !error && articles?.length === 0 && (
        <p className="text-sm text-muted-foreground py-2">لا مزيد من هذا الكاتب</p>
      )}
    </ArticleSectionCollapsible>
  );
}
