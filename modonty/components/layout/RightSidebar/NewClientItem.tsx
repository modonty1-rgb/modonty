...
export function NewClientItem({ clientName, clientSlug, clientLogo, industry, articleCount = 0 }: NewClientItemProps) {
  return (
    <PartnerRow slug={clientSlug}>
      ...
      <Link href={`/clients/${clientSlug}`} className="flex flex-1 min-w-0 items-center gap-3 py-0.5 px-1">
      ...
      {articleCount > 0 && (
        <Link
          ...
          className="me-1 inline-flex shrink-0 items-center gap-1 rounded-full border border-border bg-muted/40 px-2 py-1 text-[11px] font-semibold text-muted-foreground transition-colors hover:border-primary/50 hover:bg-primary/10 hover:text-primary"
        >
