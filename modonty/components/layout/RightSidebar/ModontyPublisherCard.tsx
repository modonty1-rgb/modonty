...
interface ModontyPublisherCardProps {
  articles: FeedPost[];
  brandLogoUrl: string | null;
  className?: string;
}

export function ModontyPublisherCard({ articles, brandLogoUrl, className }: ModontyPublisherCardProps) {
      ...
      <div className="flex items-center gap-2 px-3 pb-2 pt-3">
        <span className="relative z-10 mb-[-20px] shrink-0 translate-x-2 -translate-y-[25px] -rotate-[16deg]">
          ...
          <h2 className="relative flex h-8 w-[82px] items-center justify-center rounded-sm border-2 border-white/90 bg-white px-2 shadow-[0_8px_16px_-10px_rgba(14,6,90,0.8)]">
            {brandLogoUrl ? (
              <OptimizedImage media={asMedia(getOptimizedLogoUrl(brandLogoUrl), "مدونتي")} alt="مدونتي" width={70} height={20} sizes="70px" className="h-5 w-[70px] object-contain" />
            ) : (
