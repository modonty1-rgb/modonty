...
import type { ClientReviewWithDetails } from "../helpers/client-review-queries";
import {
  approveClientReview,
  rejectClientReview,
  deleteClientReview,
    ...
    <span className="inline-flex items-center gap-0.5" aria-label={`${rating} من 5`}>
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          key={n}
          className={`h-4 w-4 ${
            n <= rating ? "fill-[#f5a623] text-[#f5a623]" : "text-muted-foreground/30"
          }`}
        />
      ))}
    </span>
...
export function ClientReviewsTable({ reviews }: Props) {
  ...
  const counts = useMemo(
    () => ({
      all: reviews.length,
      PENDING: reviews.filter((c) => c.status === "PENDING").length,
      APPROVED: reviews.filter((c) => c.status === "APPROVED").length,
      REJECTED: reviews.filter((c) => c.status === "REJECTED").length,
      DELETED: reviews.filter((c) => c.status === "DELETED").length,
    }),
    [reviews]
  );
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full max-w-sm">
            ...
            <Input
              ...
              placeholder="ابحث في الرأي أو الكاتب..."
              className="ps-9"
            />
          </div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(isDeleted || isRejected) && (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => run(c.id, () => restoreClientReview(c.id), "تمت الاستعادة")}
                              disabled={isWorking}
                              className="gap-1.5"
                            >
                          ...
                          {!isApproved && !isDeleted && (
                            <Button
                              size="sm"
                              onClick={() => run(c.id, () => approveClientReview(c.id), "تمت الموافقة")}
                              disabled={isWorking}
